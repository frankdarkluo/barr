...
```
你是科研实验记录整理助手。根据今天的原始实验日志，按 daily_research_log_skill.md 的模板生成结构化日报。
...
2. **图片处理**：TensorBoard/WandB 截图用 `![描述](path)` 嵌入；暂无截图用 `[图：描述]` 占位。
3. **周报节奏**：建议周五晚或周一早运行，输入为该周所有日报 `.md` 文件。
4. **文件命名**：日报 `YYYY-MM-DD-daily.md`，周报 `YYYY-WXX-weekly.md`，方便脚本 glob。